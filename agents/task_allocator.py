...
class TaskAllocator:
    ...
    def get_valid_tasks(self):
        ...
        for task_id in self.world_info["tasks"].keys():
            if not(self.world_info["tasks"][task_id]["completed"]() or self.finished_tasks.keys().__contains__(task_id) or self.current_tasks.keys().__contains__(task_id)):
...
class ActorController:
    ...
    def update(self):
        ...
        if self.api.get_field(self.id, "state") == 2:  # Digging
            ...
            viable_resources = list(
                filter(lambda resource_id: self.api.get_field(resource_id, "colour") == target_colour and not resource_id in self.master.reserved_resources,
            ...
            return
        elif self.current_goal.type == Goal.FINISH_SITE and self.api.get_field(self.id, "state") == 0 and self.api.get_field(self.id, "node") == self.current_goal.parameters[0]:
    ...
    def pick_up_x_resources(self, colour, amount=1):
        ...
        viable_resources = list(
            filter(lambda resource_id: (resource_id in self.master.reserved_resources) and self.master.reserved_resources[resource_id] == (self.current_goal.task, Goal.DELIVER), viable_resources))
        ...
        while True:
            if frontier:
                ...
                for edge in self.api.get_field(current_path[0][-1], "edges"):
                    ...
                    if explored.__contains__(next_node) and explored[next_node][1] > new_path[1]:
                        explored[next_node] = new_path
                    elif not(explored.__contains__(next_node) or frontier.__contains__(next_node)) or (frontier.__contains__(next_node) and frontier[next_node][1] > new_path[1]):
                        frontier[next_node] = new_path
            else:
                return None
    ...
    def confirm_resources(self, needed_resources):
        ...
        for type, amount in enumerate(needed_resources):
            ...
            if amount - pending_resources[type] > 0:
                dig_goal = Goal(self.master.get_goal_id, Goal.DIG, [amount - pending_resources[type], type], self.current_goal.task)
                deliver_goal = Goal(self.master.get_goal_id, Goal.DELIVER, [amount - pending_resources[type], type, self.api.get_field(self.current_goal.task, "node")], self.current_goal.task)
    ...
    def deliver_resources(self, amount, colour, node):
        ...
        if destination is None:
            ...
            self.push(self.drop_resources, amount, colour, (self.current_goal.task, Goal.FINISH_SITE), include_actor=False)
            self.push(self.finish_goal, include_actor=False)
